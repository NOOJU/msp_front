...
describe('Apply Component', () => {
    ...
    test('shows error message for invalid VM name', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText((content, element) =>
                content.includes('VM 이름은 알파벳 대소문자, 숫자, -, _ 만 포함해야 하며 1~239자 이내여야 합니다.')
            )).toBeInTheDocument();
        });
    });
    ...
    test('submits the form with valid data', async () => {
        ...
        await waitFor(() => {
            expect(mockedAxios.post).toHaveBeenCalledWith('http://localhost:8000/vm-apply', expect.any(Object), expect.any(Object));
        });
